...
const INLINE_CSS = `
.management-control-wrapper.settings-management-shell{height:100%;min-height:0;display:grid!important;grid-template-columns:292px minmax(0,1fr)!important;gap:12px!important;overflow:hidden!important;padding:0!important;background:transparent!important;border:0!important}
.management-control-sidebar{height:100%;display:flex;flex-direction:column;overflow:hidden;border:1px solid #dbe7fb;border-radius:20px;background:#fff}
.management-control-sidebar-head{padding:16px 18px;border-bottom:1px solid #e5edf8}
.management-control-sidebar-head span,.sp-chip{display:block;color:#2563eb;font-size:.64rem;font-weight:900;letter-spacing:.12em;text-transform:uppercase}
.management-control-sidebar-head strong{display:block;margin-top:6px;color:#0f2746;font-size:1.02rem;font-weight:900}
.management-control-sidebar-head small{display:block;margin-top:4px;color:#64748b;font-size:.72rem;font-weight:700}
.management-control-nav-list{flex:1;display:grid;align-content:start;gap:8px;overflow:auto;padding:14px 12px}
.management-control-nav-btn{width:100%;min-height:56px;display:grid;grid-template-columns:38px minmax(0,1fr);align-items:center;gap:12px;padding:10px 13px;border:0;border-radius:16px;background:transparent;color:#0f2746;text-align:left;font-weight:900}
.management-control-nav-btn.active{color:#fff;background:linear-gradient(135deg,#2563eb,#087ea4)}
.management-control-nav-icon{width:38px;height:38px;display:grid;place-items:center;border-radius:13px;color:#2563eb;background:#eef4ff}
.management-control-nav-btn.active .management-control-nav-icon{color:#fff;background:rgba(255,255,255,.2)}
.management-control-content,.management-legacy-content{min-height:0;height:100%;overflow:hidden}
.management-legacy-content>.settings-module-root{height:100%!important;max-height:100%!important;padding:0!important;border:0!important;background:transparent!important;box-shadow:none!important}
.management-legacy-content .settings-layout{height:100%!important;grid-template-columns:1fr!important;padding:0!important}
.management-legacy-content .settings-menu{display:none!important}
.management-legacy-content .settings-content{height:100%!important;min-height:0!important}

.software-policy-module{height:100%;min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr);gap:12px;color:#0f2746;overflow:hidden}
.software-policy-module *{box-sizing:border-box}
.sp-top,.sp-panel,.sp-section{border:1px solid #dbe7fb;border-radius:20px;background:#fff;box-shadow:0 14px 30px rgba(15,23,42,.045)}
.sp-top{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:15px 18px}
.sp-top h2,.sp-panel h3{margin:3px 0;color:#0f2746;font-weight:900;letter-spacing:-.04em}
.sp-top p,.sp-help{margin:0;color:#64748b;font-size:.74rem;font-weight:700;line-height:1.45}
.sp-layout{min-height:0;display:grid;grid-template-columns:300px minmax(0,1fr);gap:12px;overflow:hidden}
.sp-panel{min-height:0;display:flex;flex-direction:column;overflow:hidden}
.sp-panel-head{flex:0 0 auto;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:14px 16px;border-bottom:1px solid #e5edf8}
.sp-panel-body{min-height:0;overflow:auto;padding:14px 16px}
.sp-rule-list{display:grid;gap:8px}
.sp-rule-card{width:100%;display:grid;gap:3px;padding:12px;border:1px solid #dbe7fb;border-radius:15px;background:#f8fbff;color:#0f2746;text-align:left;cursor:pointer}
.sp-rule-card.active{background:linear-gradient(135deg,#2563eb,#087ea4);color:#fff;border-color:transparent}
.sp-rule-card strong,.sp-rule-card span,.sp-rule-card small{display:block;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.sp-rule-card small{color:#64748b;font-size:.68rem;font-weight:750}
.sp-rule-card.active small{color:rgba(255,255,255,.82)}

.sp-work{min-height:0;height:100%;display:grid;grid-template-rows:auto minmax(0,1fr);gap:12px;overflow:hidden}
.sp-main-grid{min-height:0;display:grid;grid-template-columns:minmax(0,1fr) 380px;gap:12px;overflow:hidden}
.sp-left-stack,.sp-right-stack{min-height:0;display:grid;gap:12px;overflow:hidden}
.sp-left-stack{grid-template-rows:auto minmax(0,1fr)}
.sp-right-stack{grid-template-rows:auto minmax(0,1fr)}
.sp-section{min-height:0;display:flex;flex-direction:column;overflow:hidden}
.sp-section-title{flex:0 0 auto;padding:12px 14px;border-bottom:1px solid #eef3fb}
.sp-section-title-row{display:flex;align-items:center;justify-content:space-between;gap:12px}
.sp-section-title strong{display:block;color:#0f2746;font-size:.84rem;font-weight:900}
.sp-section-title small{display:block;margin-top:2px;color:#64748b;font-size:.68rem;font-weight:700}
.sp-section-body{min-height:0;overflow:auto;padding:14px}
.sp-rule-setup .sp-section-body{overflow:visible}
.sp-rule-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}
...
.sp-field span{color:#64748b;font-size:.62rem;font-weight:900;text-transform:uppercase}
.sp-field input,.sp-field select,.sp-field textarea,.sp-search input{width:100%;min-height:40px;border:1px solid #d7e3f5;border-radius:12px;background:#fff;color:#0f2746;padding:0 12px;font-size:.78rem;font-weight:750;outline:none}
.sp-field textarea{min-height:54px;padding:10px;resize:vertical}
.sp-btn,.sp-icon,.sp-danger{min-height:40px;display:inline-flex;align-items:center;justify-content:center;gap:8px;border-radius:12px;font-size:.76rem;font-weight:900;cursor:pointer}
.sp-btn.primary{border:0;color:#fff;background:linear-gradient(135deg,#2563eb,#087ea4);padding:0 16px}
.sp-btn.secondary{border:1px solid #d7e3f5;background:#fff;color:#2563eb;padding:0 16px}
.sp-icon{width:40px;border:1px solid #d7e3f5;background:#fff;color:#2563eb}
.sp-danger{width:36px;min-height:36px;border:1px solid #fecaca;background:#fff1f2;color:#dc2626}
.sp-btn:disabled,.sp-icon:disabled{opacity:.55;cursor:not-allowed}
.sp-action-row{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:12px}
.sp-rule-hint{margin-top:10px;padding:9px 11px;border:1px solid #dbeafe;border-radius:12px;background:#eff6ff;color:#1d4ed8;font-size:.72rem;font-weight:800}
.sp-alert{padding:10px 14px;border-radius:14px;font-size:.74rem;font-weight:850}
.sp-alert.error{color:#991b1b;background:#fef2f2;border:1px solid #fecaca}
.sp-alert.success{color:#166534;background:#f0fdf4;border:1px solid #bbf7d0}
.sp-alert.info{color:#1d4ed8;background:#eff6ff;border:1px solid #bfdbfe}

.sp-software-section{min-height:0}
.sp-software-toolbar{display:grid;grid-template-columns:200px minmax(0,1fr) auto;gap:10px;align-items:end;margin-bottom:12px}
.sp-search{min-height:40px;display:flex;align-items:center;gap:8px;border:1px solid #d7e3f5;border-radius:12px;padding:0 11px;background:#fff;color:#64748b}
.sp-search input{min-height:0;border:0;padding:0}
.sp-table{height:100%;min-height:300px;overflow:auto;border:1px solid #e5edf8;border-radius:16px}
.sp-row{min-height:54px;display:grid;grid-template-columns:42px minmax(240px,1.3fr) minmax(135px,.7fr) 78px;gap:12px;align-items:center;padding:0 14px;border-bottom:1px solid #edf2f7;font-size:.74rem;font-weight:740}
.sp-row.head{position:sticky;top:0;z-index:2;min-height:42px;background:#f3f7fc;color:#64748b;font-size:.62rem;font-weight:900;text-transform:uppercase}
.sp-row.selected{background:#eff6ff}
.sp-row strong,.sp-row small{display:block;min-width:0;overflow:hidden;text-overflow:ellipsis}
.sp-row strong{color:#0f2746}
.sp-row small{color:#64748b;font-size:.64rem;white-space:nowrap}

.sp-class-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.sp-class-btn{min-height:66px;padding:12px;border:1px solid #d7e3f5;border-radius:16px;background:#fff;color:#0f2746;text-align:left;font-weight:900}
.sp-class-btn.active.legal{border-color:#bbf7d0;background:#f0fdf4;color:#166534}
.sp-class-btn.active.illegal{border-color:#fecaca;background:#fef2f2;color:#991b1b}
.sp-selected-box{margin:10px 0;padding:10px 12px;border:1px solid #bfdbfe;border-radius:15px;background:#eff6ff;color:#1d4ed8;font-size:.76rem;font-weight:850}
.sp-saved{border-top:1px solid #e5edf8}
.sp-saved-row{min-height:56px;display:grid;grid-template-columns:minmax(0,1fr) 70px 36px;gap:8px;align-items:center;border-bottom:1px solid #edf2f7}
.sp-saved-row strong,.sp-saved-row small{display:block;min-width:0;overflow:hidden;text-overflow:ellipsis}
.sp-saved-row small{font-size:.65rem;color:#64748b}
.sp-badge{display:inline-flex;justify-content:center;align-items:center;min-height:24px;border-radius:999px;padding:0 8px;font-size:.62rem;font-weight:900}
.sp-badge.legal{color:#166534;background:#dcfce7}
.sp-badge.illegal{color:#991b1b;background:#fee2e2}
.sp-empty{min-height:150px;display:grid;place-items:center;color:#64748b;font-size:.8rem;font-weight:800;text-align:center;padding:18px}

@media(max-width:1360px){.sp-main-grid{grid-template-columns:1fr}.sp-right-stack{grid-template-columns:1fr 1fr;grid-template-rows:auto}.sp-rule-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:980px){.management-control-wrapper.settings-management-shell,.sp-layout,.sp-main-grid,.sp-right-stack,.sp-software-toolbar,.sp-form-grid,.sp-rule-grid,.sp-class-grid{grid-template-columns:1fr!important}.sp-row{grid-template-columns:42px 1fr}.sp-row.head{display:none}.sp-field.span2{grid-column:1/-1}}
`;
        ...
        <main className="sp-work">
          ...
          <div className="sp-main-grid">
            <div className="sp-left-stack">
              <section className="sp-section sp-rule-setup">
                <div className="sp-section-title sp-section-title-row">
                  <div>
                    <strong>1. Policy rule</strong>
                    <small>Set category, office hours and usage threshold.</small>
                  </div>
                  <button className="sp-btn primary" type="button" onClick={saveRule} disabled={saving}><Save size={15} /> Save Rule</button>
                </div>
                <div className="sp-section-body">
                  <div className="sp-rule-grid">
                    <label className="sp-field span2"><span>Rule name</span><input value={ruleForm.policyName} onChange={(e) => setRuleForm((c) => ({ ...c, policyName: e.target.value }))} placeholder="Example: Microsoft Office policy" /></label>
                    <label className="sp-field span2"><span>Category</span><select value={ruleForm.categoryId} onChange={(e) => handleCategoryChange(e.target.value)}><option value="">Select category</option>{categories.map((row) => <option key={row.CategoryID} value={row.CategoryID}>{row.CategoryName}</option>)}</select></label>
                    ...
                    <label className="sp-field"><span>Utilized hour/day</span><input type="number" min="0" step="0.25" value={ruleForm.utilizedHours} onChange={(e) => setRuleForm((c) => ({ ...c, utilizedHours: e.target.value }))} /></label>
                  ...
                  <div className="sp-rule-hint">Monday to Friday. ≥ {ruleForm.utilizedHours || 2} hour/day = utilized, below that = underutilized, no activity = not used.</div>
                </div>
              </section>

              <section className="sp-section sp-software-section">
                <div className="sp-section-title">
                  <strong>2. Select software</strong>
                  <small>Choose software under selected category, then apply license details from the right panel.</small>
                </div>
                <div className="sp-section-body" style={{ display: "flex", flexDirection: "column" }}>
            ...
            <aside className="sp-right-stack">
              <section className="sp-section sp-license-card">
                <div className="sp-section-title">
                  <strong>3. Classification & license</strong>
                  <small>Apply this detail to selected software.</small>
                </div>
                <div className="sp-section-body">
              ...
              <section className="sp-section">
                <div className="sp-section-title">
                  <strong>Saved software</strong>
                  <small>Software already assigned to this rule.</small>
                </div>
                <div className="sp-section-body">
