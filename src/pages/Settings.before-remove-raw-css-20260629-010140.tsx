...
import "../styles/ema-settings-layout.css";
import "../styles/ema-settings-real.css";
...
const settingsInlineCss = String.raw\`
/* SETTINGS INLINE UI - page locked, side tree locked, right content scroll */

.settings-module-root,
.settings-module-root.ema-settings-pro {
  width: 100% !important;
...
.settings-module-root *,
.settings-module-root *::before,
.settings-module-root *::after {
  box-sizing: border-box !important;
}
...
.settings-menu .panel-head strong {
...
.settings-menu .panel-head small {
  display: block !important;
...
.settings-menu-list {
  ...
  scrollbar-color: rgba(100,116,139,.36) transparent !important;
}
...
.settings-menu-list::-webkit-scrollbar-track {
...
/* TREE BUTTONS - no description, no overlap */
.setting-btn {
  width: 100% !important;
...
.setting-btn[aria-current="page"] {
  background: linear-gradient(135deg, rgba(37,99,235,.13), rgba(79,70,229,.09)) !important;
  color: #1d4ed8 !important;
...
/* hide long description inside tree */
.setting-btn small {
...
setModuleLoaded(true);
  ...
  const savePcAgingRule = async () => {
    ...
    try {
      ...
      setPcAgingLoaded(true);
      addActivity("Saved PC aging rule", `Aging threshold set to ${normalizedRule.monitorMaxYears} years Â· just now`);
  ...
  const savePricingRow = async (id: string) => {
    ...
    try {
      ...
      showToast("success", "Pricing saved", `${row.Category}${row.Brand ? ` â€¢ ${row.Brand}` : ""}${row.Model ? ` â€¢ ${row.Model}` : ""} has been saved.`);
    } catch (error) {
  ...
  const confirmDeletePricingRow = async () => {
    ...
    try {
      ...
      showToast("success", "Pricing deleted", `${row.Category || "Pricing row"}${row.Brand ? ` â€¢ ${row.Brand}` : ""}${row.Model ? ` â€¢ ${row.Model}` : ""} has been removed.`);
    } catch (error) {
  ...
  const saveManagementPolicy = async () => {
    ...
    try {
      ...
      setManagementPolicyLoaded(true);
      addActivity("Saved management policy", `${MANAGEMENT_POLICY_FIELDS.length} dashboard assumption value(s) updated Â· just now`);
  ...
  const saveSection = () => {
    ...
    addActivity(`Saved ${active.title}`, "Configuration saved to prototype audit log Â· just now");
  };
  ...
  const saveUserAccess = async () => {
    ...
    try {
      if (editingUserIndex === null) {
        ...
        addActivity("Added user access", `${createdUser.name} Â· ${createdUser.role} Â· just now`);
        ...
        addActivity("Updated user access", `${updatedUser.name} Â· ${updatedUser.role} Â· just now`);
  ...
  const confirmDeleteUser = async () => {
    ...
    try {
      ...
      addActivity("Deleted user access", `${user.name} permanently deleted from EMA_Users Â· just now`);
  ...
  return (
    <main className="ema-settings-page" data-section={activeSection}>
      ...
      <div className="ema-settings-grid">
        <aside className="ema-settings-tree">
          <div className="ema-settings-tree-head">
            <span>SETTINGS CENTER</span>
            <strong>Configuration Area</strong>
            <small>Select system setup domain</small>
          </div>

          <div id="settingsMenu" className="ema-settings-menu-list" role="tablist" aria-label="Settings navigation">
            {sectionOrder.map((key) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  data-section={key}
                  className={activeSection === key ? "ema-setting-btn is-active" : "ema-setting-btn"}
                  onClick={() => {
                    setActiveSection(key);
                    setSectionSearch("");
                  }}
                >
                  <span className="ema-setting-icon"><SettingsMenuIcon name={item.icon} /></span>
                  <span className="ema-setting-text"><strong>{item.title}</strong><small>{item.desc}</small></span>
                </button>
              );
            })}
          </div>
        ...
        <section className="ema-settings-content">
          <div className="ema-settings-hero">
            <div>
...
function PricingDeleteModal({ row, loading, onClose, onConfirm }: { row: PricingRow | null; loading: boolean; onClose: () => void; onConfirm: () => void }) {
  ...
  const label = [row.Category, row.Brand || "All Brands", row.Model || "All Models"].filter(Boolean).join(" â€¢ ");
